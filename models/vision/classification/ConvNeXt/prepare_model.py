...
Model variants (BSD-3-Clause, ImageNet-1K pretrained via torchvision):
  convnext_tiny   – 224×224,  28.59M params,  4.46G FLOPs, top-1  82.520%  [default, recommended]
  convnext_small  – 224×224,  50.22M params,  8.68G FLOPs, top-1  83.616%
  convnext_base   – 224×224,  88.59M params, 15.36G FLOPs, top-1  84.062%
  convnext_large  – 224×224, 197.77M params, 34.36G FLOPs, top-1  84.414%
...
def print_model_table() -> None:
    ...
    for key, info in MODEL_CATALOG.items():
        h, w = info["shape"]
        print(
            f"  {key:<16}  {info['backbone']:<16}  {h}×{w:<5}  "
